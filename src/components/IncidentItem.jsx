...
export const IncidentItem = ({ incident, onAcknowledge, onResolve }) => {
  ...
  return (
    <div className={`relative ${severityStyles[incident.severity]} border rounded-2xl p-4 transition-all duration-300 animate-slide-in ${incident.acknowledged ? 'opacity-60' : ''}`}>
      ...
      <div className="flex items-center justify-end gap-2 mt-4 pt-4 border-t border-white/5">
        {!incident.acknowledged && (
          <button
            ...
            className="px-3 py-1.5 text-xs font-medium text-white bg-blue-500/20 hover:bg-blue-500/30 border border-blue-500/30 rounded-lg transition-colors"
          >
            Acknowledge
          </button>
        )}
        <button
          ...
          className="px-3 py-1.5 text-xs font-medium text-white bg-gray-700 hover:bg-gray-600 border border-gray-600 rounded-lg transition-colors"
        >
          {incident.acknowledged ? 'Resolve' : 'Dismiss'}
        </button>
      </div>

      {/* Pulse indicator for unacknowledged critical */}
