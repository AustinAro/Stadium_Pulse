    ...
    <div className="bg-card border border-border rounded-2xl p-5 flex flex-col justify-between h-full">
      <div>
        <h3 className="text-md font-bold text-text-primary mb-1 flex items-center gap-2">
          <svg className="w-5 h-5 text-accent animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
          </svg>
          Stadium Spatial Minimap
        </h3>
        <p className="text-xs text-text-muted mb-4">Click sectors to locate and focus. Hover to preview.</p>
      </div>

      <div className="grid grid-cols-4 grid-rows-5 gap-2 bg-slate-950/60 p-4 rounded-xl border border-border/50 w-full aspect-square max-w-[240px] mx-auto select-none" role="img" aria-label="Interactive map representation of stadium sectors">
        {layout.map(item => {
          ...
          return (
            ...
            >
              <span className="truncate max-w-[55px]">{item.name.replace(" Section", "").replace(" Zone", "")}</span>
...
export const Dashboard = () => {
  ...
  const exportIncidentsToCSV = () => {
    ...
    const headers = ["Incident ID", "Sector Zone", "Alert Type", "Severity", "Message Description", "Assigned Staff", "Timestamp", "Acknowledged"];
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 p-4 sm:p-6" role="main">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-text-primary flex items-center gap-3">
            <span className="relative">
              <svg className="w-8 h-8 text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.986-7C14D003 6.001 14.986 9 15 10c0 2-.5 5-2.343 8.657z" />
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              </svg>
            </span>
            StadiumPulse
          </h1>
          <p className="text-text-muted text-sm mt-1">Real-time stadium operations & crowd intelligence</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          >
            {isMuted ? (
              ...
              <svg className="w-4 h-4 text-accent animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            ...
            <span className="text-xs font-semibold uppercase">{isMuted ? "Muted" : "Sound On"}</span>
          </button>
          ...
          <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-card border border-border rounded-lg select-none">
            ...
            <span className="text-sm text-text-secondary font-medium" aria-label={`Logged in coordinator: ${coordinator}`}>{coordinator}</span>
          </div>
        </div>
      </header>

      {/* Dynamic Copilot & Spatial Map Panel */}
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6" aria-label="AI operations panel and map">
        <div className="lg:col-span-2">
          {aiInsights ? (
            <div className="bg-card border border-border rounded-2xl p-5 flex flex-col justify-between h-full relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-r from-accent/5 to-transparent pointer-events-none" aria-hidden="true" />
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-3.5 bg-slate-950/40 border border-border rounded-xl">
                    <span className="text-[10px] text-text-muted uppercase tracking-wider font-bold block mb-1">Predicted Peak sector</span>
                    <span className="text-sm font-semibold text-text-primary block">{aiInsights.predictedPeak}</span>
                  </div>
                  <div className="p-3.5 bg-slate-950/40 border border-border rounded-xl">
                    <span className="text-[10px] text-text-muted uppercase tracking-wider font-bold block mb-1">Crowd Flow Surge Risk</span>
                    <span className={`text-sm font-bold block ${aiInsights.riskColor}`}>{aiInsights.crowdFlowRisk}</span>
                  </div>
                </div>

                <div className="mt-4 p-3.5 bg-slate-950/40 border border-border rounded-xl">
                  <span className="text-[10px] text-text-muted uppercase tracking-wider font-bold block mb-1">Resource Optimization Plan</span>
                  <span className="text-xs text-text-secondary leading-relaxed block">{aiInsights.resourceOptimization}</span>
                </div>

                <div className="mt-4 p-3.5 bg-accent/5 border border-accent/15 rounded-xl flex items-start gap-3">
                  <svg className="w-5 h-5 text-accent flex-shrink-0 mt-0.5 animate-bounce-subtle" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
                  </svg>
                  <div>
                    <span className="text-[10px] text-accent uppercase tracking-wider font-bold block mb-0.5">Recommended Action Plan</span>
                    <span className="text-xs text-text-primary leading-relaxed font-semibold block">{aiInsights.recommendedAction}</span>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-5 flex items-center justify-center h-full">
              <span className="text-xs text-text-muted">Analyzing real-time stadium metrics...</span>
            </div>
          )}
        </div>
      ...
      {/* Stats Bar */}
      <section className="grid grid-cols-2 sm:grid-cols-4 gap-3 select-none" aria-label="Operations metrics summary">
        <StatCard 
          label="Total Zones" 
          value={zones.length} 
          icon={<ZonesIcon />} 
      ...
      {activeIncidents.length > 0 && (
        <section className="bg-card border border-border rounded-2xl overflow-hidden animate-slide-in" aria-labelledby="incidents-heading">
          <div className="p-4 bg-card bg-opacity-50 border-b border-border flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 select-none">
            <h2 id="incidents-heading" className="text-lg font-semibold text-text-primary flex items-center gap-2">
      ...
      <section aria-labelledby="zones-heading">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4 select-none">
          <h2 id="zones-heading" className="text-xl font-bold text-text-primary">Stadium Zones</h2>
      ...
      {showShortcutsHelp && (
        <div className="fixed inset-0 z-50 bg-background/85 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in" role="dialog" aria-modal="true" aria-labelledby="modal-heading">
          <div className="bg-card border border-border rounded-2xl max-w-md w-full p-6 shadow-2xl relative select-none">
            ...
            <h3 id="modal-heading" className="text-md font-bold text-text-primary mb-4 flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs mb-5">
              <div className="border border-border/50 p-2 rounded-lg bg-slate-950/40 flex justify-between items-center">
                ...
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">N</kbd>
              </div>
              <div className="border border-border/50 p-2 rounded-lg bg-slate-950/40 flex justify-between items-center">
                <span className="text-text-secondary">East Gate</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">E</kbd>
              </div>
              <div className="border border-border/50 p-2 rounded-lg bg-slate-950/40 flex justify-between items-center">
                <span className="text-text-secondary">South Gate</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">G</kbd>
              </div>
              <div className="border border-border/50 p-2 rounded-lg bg-slate-950/40 flex justify-between items-center">
                <span className="text-text-secondary">West Gate</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">W</kbd>
              </div>
              <div className="border border-border/50 p-2 rounded-lg bg-slate-950/40 flex justify-between items-center">
                <span className="text-text-secondary">VIP Lounge</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">V</kbd>
              </div>
              <div className="border border-border/50 p-2 rounded-lg bg-slate-950/40 flex justify-between items-center">
                <span className="text-text-secondary">Press Box</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">P</kbd>
              </div>
              <div className="border border-border/50 p-2 rounded-lg bg-slate-950/40 flex justify-between items-center">
                <span className="text-text-secondary">Family Zone</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">Y</kbd>
              </div>
              <div className="border border-border/50 p-2 rounded-lg bg-slate-950/40 flex justify-between items-center">
                <span className="text-text-secondary">Supporters Section</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">S</kbd>
              </div>
              <div className="border border-border/50 p-2 rounded-lg bg-slate-950/40 flex justify-between items-center">
                <span className="text-text-secondary">Concourse A</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">A</kbd>
              </div>
              <div className="border border-border/50 p-2 rounded-lg bg-slate-950/40 flex justify-between items-center">
                <span className="text-text-secondary">Concourse B</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">B</kbd>
              </div>
              <div className="border border-border/50 p-2 rounded-lg bg-slate-950/40 flex justify-between items-center">
                <span className="text-text-secondary">Food Court</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">F</kbd>
              </div>
              <div className="border border-border/50 p-2 rounded-lg bg-slate-950/40 flex justify-between items-center">
                <span className="text-text-secondary">Merchandise</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">M</kbd>
              </div>
            </div>
            ...
            <div className="border-t border-border pt-4 text-xs text-text-muted space-y-2">
              <div className="flex justify-between items-center">
                ...
                <kbd className="px-2 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">SPACE</kbd>
              </div>
              <div className="flex justify-between items-center">
                <span>Clear selection highlight</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">ESC</kbd>
              </div>
              <div className="flex justify-between items-center">
                <span>Toggle help overlay modal</span>
                <kbd className="px-1.5 py-0.5 bg-slate-800 text-accent rounded text-[10px] font-mono font-bold">?</kbd>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Toast Notification */}
      {toastMsg && (
        <div className="fixed bottom-5 right-5 z-50 bg-slate-900 border border-accent/35 rounded-xl px-4 py-2.5 shadow-glow flex items-center gap-2.5 text-xs font-semibold text-accent animate-fade-in select-none" role="status" aria-live="polite">
          <span className="w-2.5 h-2.5 bg-accent rounded-full animate-ping absolute -top-1 -right-1" aria-hidden="true" />
