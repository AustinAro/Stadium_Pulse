...
export const MetricCard = ({ zone }) => {
  ...
  const hasActiveIncident = incidents.some((inc) => inc.zone === zone.name && !inc.resolved);
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <h3 className="text-lg font-semibold text-text-primary flex items-center gap-1.5">
          ...
          {keyLabel && (
            <kbd
              className="px-1.5 py-0.5 text-[9px] font-bold bg-slate-800 text-accent border border-accent/20 rounded font-mono select-none"
              title={`Press ${keyLabel} shortcut key`}
            >
              {keyLabel}
            </kbd>
          )}
      ...
      <div className="flex items-baseline justify-between mb-4">
        ...
        <div className="flex items-baseline gap-1">
          <span
            className={`text-2xl font-bold ${isHighWait ? 'text-red-400' : 'text-text-primary'}`}
          >
            {zone.queueTime}
          </span>
      ...
      {hasActiveIncident && (
        <div className="relative pt-4 border-t border-red-500/20 animate-pulse-subtle">
          <div className="flex items-center gap-2 text-red-400 text-sm">
            <svg
              className="w-4 h-4 flex-shrink-0 animate-bounce-subtle"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
              aria-hidden="true"
